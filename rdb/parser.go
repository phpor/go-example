...
func (d *decode) decode() error {
	...
	ignore := make([]byte, 4)
	io.ReadFull(d.r, ignore)

	ignorelen := int64(binary.LittleEndian.Uint32(ignore))
	io.ReadFull(d.r, make([]byte, ignorelen))
	d.event.StartRDB()
...
func (d *decode) readZipmap(key []byte, expiry int64) error {
	...
	b, err := buf.ReadByte()
	s := []byte{a, b}
	length := int(binary.BigEndian.Uint16(s))
	d.event.StartHash(key, int64(length), expiry)
	for {
		next_length := readZipmapNextLength(buf)
		if next_length == -1 {
			break
		}
		field := make([]byte, next_length)
		io.ReadFull(buf, field)
		next_length = readZipmapNextLength(buf)
		if next_length == -1 {
			panic("Unexepcted end of zip map")
		}
		lenFreeByte, _ := buf.ReadByte()
		value := make([]byte, next_length)
		io.ReadFull(buf, value)
		buf.Seek(int64(lenFreeByte), 1)
		d.event.Hset(key, field, value)
		/*
...
func readZipmapNextLength(buf *sliceBuffer) int64{
	lenByte, _ := buf.ReadByte()
	if lenByte < 254 {
		return int64(lenByte)
	}else if lenByte == 254 {
		lenByteArr, _ := buf.Slice(4)
		return int64(binary.LittleEndian.Uint32(lenByteArr))
	}
	return -1;
}
